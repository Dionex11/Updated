...
//reading calibrated samples
uint8_t max32664::readCalibSamples(){

  uint8_t    ret = writeByte(0x00, 0x00); //reading sensor hub status 
  if(!ret){
    //Serial.println("failed to read status, could not read samples !!!");
    return 0;
  }
  ...
  //calsysval.writeValue(readBuff[13]);
  return readBuff[12];
}
...
// function declaration to make sensor to enter to application mode
...
// fun declaration to read number of samples
...
bool max32664::configAlgoInEstimationMode(){
  ...
  ret = writeByte(0x10, 0x00, 0x03); // Set output mode to sensor + algorithm data.
  if(!ret) return false;
  delay(30);
...
// function declaration for hub begin
uint8_t max32664::hubBegin(){
  ...
  readSensorHubVersion(); // calling to write sensor hub version
  uint8_t responseByte = readByte(READ_DEVICE_MODE, 0x00); // 0x00 only possible Index Byte.
  
  return responseByte; 
}

// function declaration for returning sensor hub version
